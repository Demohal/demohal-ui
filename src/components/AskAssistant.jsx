/* src/components/AskAssistant.jsx
   Modular shell (self-contained): restores full behavior for Ask, Demos, Docs, Price, Meeting.
   Backend: https://demohal-app-dev.onrender.com
...
const UI = {
  ...
  BTN:
    "w-full text-center rounded-xl px-4 py-3 shadow transition-colors text-[var(--btn-fg)] border border-[var(--btn-border)] " +
    "bg-gradient-to-b from-[var(--btn-grad-from)] to-[var(--btn-grad-to)] hover:from-[var(--btn-grad-from-hover)] hover:to-[var(--btn-grad-to-hover)]",
  BTN_DOCS:
    "w-full text-center rounded-xl px-4 py-3 shadow transition-colors text-[var(--btn-fg)] border border-[var(--btn-border)] " +
    "bg-gradient-to-b from-[var(--btn-docs-grad-from)] to-[var(--btn-docs-grad-to)] hover:from-[var(--btn-docs-grad-from-hover)] hover:to-[var(--btn-docs-grad-to-hover)]",
  FIELD:
    "w-full rounded-lg px-4 py-3 text-base bg-[var(--field-bg)] border border-[var(--field-border)]",
  TAB_ACTIVE:
    "px-4 py-1.5 text-sm font-medium whitespace-nowrap flex-none transition-colors rounded-t-md border border-b-0 " +
    "bg-[var(--tab-active-bg)] text-[var(--tab-active-fg)] border-[var(--tab-active-border)] -mb-px shadow-[var(--tab-active-shadow)]",
  TAB_INACTIVE:
    "px-4 py-1.5 text-sm font-medium whitespace-nowrap flex-none transition-colors rounded-t-md border border-b-0 " +
    "text-[var(--tab-inactive-fg)] border-[var(--tab-inactive-border)] bg-gradient-to-b " +
    "from-[var(--tab-inactive-grad-from)] to-[var(--tab-inactive-grad-to)] hover:from-[var(--tab-inactive-hover-from)] hover:to-[var(--tab-inactive-hover-to)] " +
...
const renderMirror = (tpl, lbl) => (tpl || "").split("{{answer_label_lower}}").join(lbl.toLowerCase()).split("{{answer_label}}").join(lbl);
...
function OptionButton({ opt, selected, onClick }) {
  return (
    <button onClick={() => onClick(opt)} className={classNames(UI.BTN, selected && "ring-2 ring-white/60")} title={opt.tooltip || ""}>
...
function PriceMirror({ lines }) {
  ...
  return (
    <div className="mb-3">
      {lines.map((ln, i) => (
        <div key={i} className="text-base italic text-gray-700 whitespace-pre-line">{ln}</div>
      ))}
    </div>
  );
}
      ...
      <div className={UI.CARD}>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-black font-bold text-lg">
            {estimate.currency_code} {Number(estimate.total_min).toLocaleString()} – {estimate.currency_code} {Number(estimate.total_max).toLocaleString()}
          </div>
        </div>
          ...
          {(estimate.line_items || []).map((li) => (
            <div key={li.product.id} className="border rounded-lg p-3">
              <div className="flex items-center justify-between">
                ...
                <div className="text-black font-bold text-lg">
                  {li.currency_code} {Number(li.price_min).toLocaleString()} – {li.currency_code} {Number(li.price_max).toLocaleString()}
                </div>
              </div>
              {Array.isArray(li.features) && li.features.length > 0 && (
                <div className="mt-2">
                  {li.features.filter((f) => f.is_standard).map((f, idx) => (
                    <span key={`${li.product.id}-${idx}`} className="inline-block text-xs border border-gray-300 rounded-full px-2 py-0.5 mr-1 mb-1">
                      {f.name}
                    </span>
                  ))}
                </div>
...
export default function AskAssistant() {
  ...
  const defaultAlias = (import.meta.env.VITE_DEFAULT_ALIAS || "demo").trim();
  ...
  const [items, setItems] = useState([]); // Ask suggested demos
  ...
  // Gate UI until brand loads (if bot will be resolved)
  const initialBrandReady = useMemo(() => !(botIdFromUrl || alias || defaultAlias), [botIdFromUrl, alias, defaultAlias]);
  ...
  /* ---------- BOT SETTINGS (alias or default) ---------- */
  useEffect(() => {
    if (botId) return;
    const whichAlias = alias || defaultAlias;
    if (!whichAlias) return;
    let cancel = false;
    (async () => {
      try {
        const res = await fetch(`${apiBase}/bot-settings?alias=${encodeURIComponent(whichAlias)}`);
  ...
  async function openMeeting() {
    ...
    try {
      ...
      if (ag && String(ag.calendar_link_type || "").toLowerCase() === "external" && ag.calendar_link) {
    ...
    return (
      ...
      >
        <div className="text-gray-800 text-center space-y-2">
          ...
          <div className="text-sm text-gray-600">Resolving default bot…</div>
        </div>
      </div>
    );
  }
            ...
            {mode === "meeting" ? (
              ...
              <div className="w-full flex-1 flex flex-col">
                ...
                <div className="text-left mt-2">
                  {loading ? <p className="text-gray-500 font-semibold animate-pulse">Thinking…</p> : lastQuestion ? (
...
/* ===== REVISION =====
   Date: 2025-09-01
   Notes: Restores full functionality in a single component; tabs anchored; intro video from /bot-settings;
   logo from /brand; price flow; meeting embed; ask bar always visible.
*/
/* src/components/AskAssistant.jsx
