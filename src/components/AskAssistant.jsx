  ...
  return (
    ...
    >
      {brandingMode ? (
        <>
          {/* Left control rail */}
          <div className="fixed top-20 z-[9999] bg-white/90 backdrop-blur-sm border rounded-xl shadow p-4 w-72 space-y-3 max-h-[75vh] overflow-auto" style={{ left: "calc(50% - 360px - 18rem - 8px)" }}>
          ...
          <div className="fixed top-20 z-[9999] bg-white/90 backdrop-blur-sm border rounded-xl shadow p-4 w-72 space-y-2 max-h-[75vh] overflow-auto" style={{ left: "calc(50% + 360px + 8px)" }}>
            ...
            <label className="flex items-center justify-between text-sm text-black">Tab Titles <input type="color" value={brandDraft.css_vars["--tab-active-fg"] || themeVars["--tab-active-fg"]} onChange={(e)=>updateCssVar("--tab-active-fg", e.target.value)} /></label>
            <label className="flex items-center justify-between text-sm text-black">Tab Background <input type="color" value={brandDraft.css_vars["--tab-active-bg"] || themeVars["--tab-active-bg"]} onChange={(e)=>updateCssVar("--tab-active-bg", e.target.value)} /></label>
            <label className="flex items-center justify-between text-sm text-black">Card Background <input type="color" value={brandDraft.css_vars["--card-bg"] || themeVars["--card-bg"]} onChange={(e)=>updateCssVar("--card-bg", e.target.value)} /></label>
            <label className="flex items-center justify-between text-sm text-black">Message Field BG <input type="color" value={brandDraft.css_vars["--field-bg"] || themeVars["--field-bg"]} onChange={(e)=>updateCssVar("--field-bg", e.target.value)} /></label>
            <label className="flex items-center justify-between text-sm text-black">Message Field Border <input type="color" value={brandDraft.css_vars["--field-border"] || themeVars["--field-border"]} onChange={(e)=>updateCssVar("--field-border", e.target.value)} /></label>
            <label className="flex items-center justify-between text-sm text-black">Send Button <input type="color" value={brandDraft.css_vars["--send-color"] || themeVars["--send-color"]} onChange={(e)=>updateCssVar("--send-color", e.target.value)} /></label>
            <label className="flex items-center justify-between text-sm text-black">Send Hover <input type="color" value={brandDraft.css_vars["--send-color-hover"] || themeVars["--send-color-hover"]} onChange={(e)=>updateCssVar("--send-color-hover", e.target.value)} /></label>
          </div>
        </>
      ) : null}
