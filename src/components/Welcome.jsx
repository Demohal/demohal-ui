...
function scoreDemo(q, d) {
  const qTokens = new Set((q||"").toLowerCase().match(/[a-z0-9]{3,}/g)||[]);
  const textTokens = ((d.title||"")+" "+(d.description||"")).toLowerCase().match(/[a-z0-9]{3,}/g)||[];
  const dTokens = new Set(textTokens); let overlap=0; qTokens.forEach(t=>{ if(dTokens.has(t)) overlap++;});
  return overlap;
}
function pruneDemoButtons(q, buttons) {
  if (!Array.isArray(buttons)||buttons.length<=2) return buttons;
  const scored = buttons.map(b=>({b,s:scoreDemo(q,b)})).sort((a,b)=>b.s-a.s);
  const top=scored[0].s; const second=scored[1]?.s??0;
  if (top < DEMO_STRONG_THRESHOLD) return scored.slice(0, Math.min(DEMO_PRUNE_MAX, buttons.length)).map(x=>x.b);
  if (second===0 || top>=second*DEMO_STRONG_RATIO) return [scored[0].b];
  const cap = Math.min(Math.max(DEMO_SECONDARY_KEEP,2), DEMO_PRUNE_MAX, scored.length);
...
function useFloatingPos(frameRef, side="left", width=460, gap=12) {
  ...
  useEffect(()=>{
    function update(){
      ...
      if(side==="left") setPos({left:Math.max(8,r.left-width-gap),top:Math.max(8,r.top+8),width});
      else setPos({left:Math.min(window.innerWidth-width-8,r.right+gap),top:Math.max(8,r.top+8),width});
    }
    update();
    const h=()=>update();
    window.addEventListener("resize",h); window.addEventListener("scroll",h,{passive:true});
...
function ThemeLabColorBox({ apiBase, botId, frameRef, onVars, sharedAuth }) {
  const MAP = {
    "banner.background":"--banner-bg","banner.foreground":"--banner-fg","page.background":"--page-bg",
    "content.area.background":"--card-bg","message.text.foreground":"--message-fg","helper.text.foreground":"--helper-fg",
    "mirror.text.foreground":"--mirror-fg","tab.background":"--tab-bg","tab.foreground":"--tab-fg",
    "demo.button.background":"--demo-button-bg","demo.button.foreground":"--demo-button-fg",
    "doc.button.background":"--doc-button-bg","doc.button.foreground":"--doc-button-fg",
    "price.button.background":"--price-button-bg","price.button.foreground":"--price-button-fg",
    "send.button.background":"--send-color","border.default":"--border-default",
  };
  const ORDER=[
  ...
  const [rows,setRows]=useState([]); const [values,setValues]=useState({});
  const [busy,setBusy]=useState(false); const [msg,setMsg]=useState("");
  function fx(url, init={}) {
  ...
  async function load(){
    try{
      const r=await fx(`${apiBase}/brand/client-tokens?bot_id=${encodeURIComponent(botId)}`);
      const j=await r.json(); const toks=(j?.ok?j.tokens:[])||[];
      setRows(toks);
      const v={}; toks.forEach(t=>v[t.token_key]=t.value||"#000000");
      setValues(v);
      const patch={}; toks.forEach(t=>{ if (MAP[t.token_key]) patch[MAP[t.token_key]]=v[t.token_key];});
      onVars && onVars(patch);
    }catch{}
  }
  useEffect(()=>{ if(sharedAuth.state==="ok") load(); },[sharedAuth.state]);
  ...
  async function doSave(){
    try{
      ...
      const r=await fx(`${apiBase}/brand/client-tokens/save`,{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({bot_id:botId,updates})
      });
      const j=await r.json();
      if(!j?.ok) throw new Error();
      setMsg(`Saved ${j.updated} token(s).`); setTimeout(()=>setMsg(""),1600);
    }catch{
      setMsg("Save failed."); setTimeout(()=>setMsg(""),1600);
    }finally{ setBusy(false); }
  }
  async function doReset(){ await load(); setMsg("Restored."); setTimeout(()=>setMsg(""),1400); }
  const grouped=useMemo(()=>{
    const m=new Map();
    rows.forEach(r=>{ const k=r.screen_key||"welcome"; if(!m.has(k)) m.set(k,[]); m.get(k).push(r);});
    ORDER.forEach(o=>{ if(m.has(o.key)) m.get(o.key).sort((a,b)=>String(a.label||"").localeCompare(String(b.label||"")));});
    return m;
  },[rows]);
  return (
    <div style={{position:"fixed",left:pos.left,top:pos.top,width:pos.width,background:"#fff",border:"1px solid rgba(0,0,0,0.2)",borderRadius:12,padding:12,zIndex:50,boxShadow:"0 8px 24px rgba(0,0,0,0.08)"}}>
                ...
                {(grouped.get(o.key)||[]).map(t=>(
                  <div key={t.token_key} className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <input type="color" value={values[t.token_key]||"#000000"} onChange={e=>updateToken(t.token_key,e.target.value)} style={{width:32,height:24,borderRadius:6,border:"1px solid rgba(0,0,0,0.2)"}}/>
      ...
      ) : sharedAuth.state==="checking" ? <div className="text-sm text-gray-600">Checking access…</div>
        : sharedAuth.state==="need_password" ? <div className="text-xs text-gray-600">Enter password in Wording panel.</div>
        : sharedAuth.state==="disabled" ? <div className="text-xs text-gray-600">ThemeLab disabled.</div>
...
function ThemeLabWordingBox({ apiBase, botId, frameRef, sharedAuth, onFormfillChange, onLiveMessages }) {
  ...
  const [options,setOptions]=useState({
    show_browse_demos:false,show_browse_docs:false,show_price_estimate:false,
  ...
  function initialKey(ms){ if(ms?.welcome_message!==undefined) return "welcome_message"; const k=Object.keys(ms||{}); return k[0]||""; }
  function mergeSynonymInto(map, f) {
    const rawKey=f.field_key;
    const canonical=FIELD_SYNONYMS[rawKey] || rawKey;
    const existing=map.get(canonical);
    const norm = {
      field_key: canonical,
      label: CANON_LABELS[canonical] || f.label || canonical.replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase()),
    ...
    if (!existing) {
      map.set(canonical, norm);
    } else {
      existing.is_collected = existing.is_collected || norm.is_collected;
      existing.is_required = existing.is_required || norm.is_required;
      if (!existing.tooltip && norm.tooltip) existing.tooltip=norm.tooltip;
      if (!existing.placeholder && norm.placeholder) existing.placeholder=norm.placeholder;
      if (!existing.field_type && norm.field_type) existing.field_type=norm.field_type;
      if (!existing.options && norm.options) existing.options=norm.options;
      if (norm.is_pii) existing.is_pii = true;
    }
  }
  async function load() {
    setLoading(true);
    try{
      const r=await fx(`${apiBase}/themelab/wording-options?bot_id=${encodeURIComponent(botId)}`);
      const j=await r.json();
      if(!j?.ok) throw new Error();
      setOptions(j.options||{});
      setMessages(j.messages||{});
      const raw=(j.standard_fields||[]).map(f=>({...f,is_collected:!!f.is_collected,is_required:!!f.is_required}));
      const map=new Map();
      raw.forEach(f=>{
      ...
      if (map.has("perspective")) {
        ...
        pf.options = pf.options && pf.options.length ? pf.options : PERSPECTIVE_OPTIONS;
        map.set("perspective",pf);
      }
      const fields=[...map.values()];
      setStandardFields(fields);
      stashRef.current={options:j.options||{},messages:j.messages||{},standard_fields:fields};
      const k=initialKey(j.messages);
      setEditingKey(k); setDraft(k?j.messages[k]||"": "");
      setDirty(false);
      propagateFormfill(j.options?.show_formfill, fields);
      onLiveMessages && onLiveMessages(j.messages||{});
    }catch{
      setMsg("Load failed."); setTimeout(()=>setMsg(""),2200);
  ...
  async function doLogin(e){
    ...
    try{
      ...
      if(r.status===200 && j?.ok){
        sharedAuth.set({state:"ok"}); if(j.token) sharedAuth.setToken(j.token); setPassword(""); await load();
      } else if (r.status===403) {
        sharedAuth.set({state:"disabled"});
      } else {
        setAuthError("Invalid password."); sharedAuth.set({state:"need_password"});
      }
    }catch{
      setAuthError("Login failed."); sharedAuth.set({state:"error"});
    }
  }
  function toggleOption(k){
    setOptions(p=>{
      const n={...p,[k]:!p[k]}; markDirty();
      if(k==="show_formfill") propagateFormfill(!p[k],standardFields);
      return n;
    });
  }
  function toggleCollected(fk){
    setStandardFields(p=>{
      const n=p.map(f=>f.field_key===fk?{...f,is_collected:!f.is_collected}:f);
      markDirty(); propagateFormfill(options.show_formfill,n); return n;
    });
  }
  function toggleRequired(fk){
    setStandardFields(p=>{
      const n=p.map(f=>f.field_key===fk?{...f,is_required:!f.is_required}:f);
      markDirty(); propagateFormfill(options.show_formfill,n); return n;
    });
  }
  function beginEdit(k){ setEditingKey(k); setDraft(messages[k]||"");}
  function applyDraft(){
    if(!editingKey) return;
    setMessages(p=>{
      const u={...p,[editingKey]:draft}; onLiveMessages && onLiveMessages(u); return u;
    }); markDirty();
  }
  function cancelEdit(){ if(editingKey) setDraft(messages[editingKey]||""); }
  async function doSave(){
    try{
      setSaving(true);
      const payload={
      ...
      const j=await r.json();
      if(!j?.ok) throw new Error();
      setMsg("Saved."); setDirty(false); stashRef.current=payload; setTimeout(()=>setMsg(""),1500);
    }catch{
      setMsg("Save failed."); setTimeout(()=>setMsg(""),2200);
    }finally{ setSaving(false); }
  }
  function doReset(){
    if(!stashRef.current){ load(); return; }
    const snap=stashRef.current;
    setOptions(snap.options); setMessages(snap.messages); setStandardFields(snap.standard_fields);
    const k=initialKey(snap.messages); setEditingKey(k); setDraft(k?snap.messages[k]||"":"");
    setDirty(false); propagateFormfill(snap.options.show_formfill,snap.standard_fields);
    setMsg("Restored."); setTimeout(()=>setMsg(""),1400); onLiveMessages && onLiveMessages(snap.messages||{});
  }
  return (
    <div style={{position:"fixed",left:pos.left,top:pos.top,width:pos.width,maxHeight:"90vh",overflowY:"auto",background:"#fff",border:"1px solid rgba(0,0,0,0.2)",borderRadius:12,padding:14,zIndex:50,boxShadow:"0 8px 24px rgba(0,0,0,0.08)"}}>
      <div className="text-base font-bold mb-2">ThemeLab Wording &amp; Options</div>
      {sharedAuth.state==="checking" && <div className="text-sm text-gray-600">Checking access…</div>}
      {sharedAuth.state==="disabled" && <div className="text-sm text-gray-600">ThemeLab is disabled for this bot.</div>}
      {sharedAuth.state==="need_password" && (
        <form onSubmit={doLogin} className="flex items-center gap-2 mb-3">
          <input type="password" placeholder="ThemeLab password"
            className="flex-1 rounded-[12px] border border-black/30 px-3 py-2 text-sm"
            value={password} onChange={e=>setPassword(e.target.value)}/>
          <button type="submit" className="px-3 py-2 rounded-[12px] bg-black text-white text-sm">Unlock</button>
          {authError && <div className="text-xs text-red-600 ml-2">{authError}</div>}
        </form>
      )}
      {sharedAuth.state==="error" && <div className="text-sm text-red-600">Unable to verify access.</div>}
      {sharedAuth.state==="ok" && (
        <>
            ...
            <div>
              ...
              <div className="space-y-1">
                {[
                  ...
                  <label key={k} className="flex items-center justify-between text-xs gap-2 cursor-pointer">
                    <span>{label}</span>
                    <input type="checkbox" checked={!!options[k]} onChange={()=>toggleOption(k)}/>
                  </label>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="space-y-1">
                {standardFields.map(f=>(
                  <div key={f.field_key} className="flex items-center justify-between text-xs gap-2">
                    <span className="flex-1 truncate">{f.label || f.field_key}</span>
                    <div className="flex items-center gap-2">
                      <input type="checkbox" title="Collect" checked={!!f.is_collected} onChange={()=>toggleCollected(f.field_key)}/>
                      <span className="text-[10px] uppercase tracking-wide opacity-70">reqd</span>
                      <input type="checkbox" title="Required" checked={!!f.is_required} onChange={()=>toggleRequired(f.field_key)}/>
                    </div>
                  </div>
                ))}
                {standardFields.length===0 && <div className="text-[10px] italic text-gray-500">No fields defined.</div>}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1 mb-3">
            {Object.entries(messageLabels).map(([k,label])=>{
              ...
              return (
                <div key={k}
                  className={classNames("flex items-center justify-between text-xs px-2 py-1 rounded cursor-pointer group",
                  ...
                  <button type="button"
                    ...
                    className={classNames("p-1 rounded transition",
                      active?"bg-white text-black":"bg-gray-200 text-gray-700 group-hover:bg-gray-300")}
                    title="Edit">
          ...
          {editingKey && (
            <div className="mb-3">
              <div className="flex items-center justify-between mb-1">
                <div className="text-xs font-semibold">Editing: {messageLabels[editingKey]}</div>
                {dirty && <div className="text-[10px] text-orange-600">Unsaved changes</div>}
              </div>
              <textarea rows={5}
                className="w-full border border-black/30 rounded p-2 text-xs"
                value={draft} onChange={e=>setDraft(e.target.value)} placeholder="Edit message text..."/>
              <div className="flex items-center justify-end gap-2 mt-1">
                <button onClick={cancelEdit} className="px-2 py-1 rounded bg-gray-200 text-xs">Revert</button>
                <button onClick={applyDraft} className="px-2 py-1 rounded bg-black text-white text-xs">Apply</button>
              </div>
            </div>
          )}
          <div className="mb-3">
            <label className="text-xs font-semibold block mb-1">Intro Video URL</label>
            <input type="text" value={options.intro_video_url||""}
              onChange={e=>{setOptions(p=>({...p,intro_video_url:e.target.value})); markDirty();}}
              className="w-full text-xs border border-black/30 rounded px-2 py-1" placeholder="https://..."/>
          </div>
          <div className="flex items-center justify-between">
            <div className="text-xs text-gray-600">{msg || (dirty?"Unsaved changes": saving?"Saving…":"")}</div>
            <div className="flex items-center gap-2">
              <button onClick={doReset} disabled={saving} className="px-3 py-1 rounded-[12px] border border-black/20 bg-white text-xs">Reset</button>
              <button onClick={doSave} disabled={saving||!dirty}
                className={classNames("px-3 py-1 rounded-[12px] text-xs",
                  dirty?"bg-black text-white":"bg-gray-300 text-gray-600 cursor-not-allowed")}>Save</button>
            </div>
          </div>
        </>
      )}
...
function ThemeLabPanels({ apiBase, botId, frameRef, onVars, onFormfillChange, onLiveMessages }) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        const r=await fetch(`${apiBase}/themelab/status?bot_id=${encodeURIComponent(botId)}`,{credentials:"include"});
        if(cancel) return;
      ...
      }catch{ if(!cancel) setAuthState("error");}
    })();
    return ()=>{cancel=true;};
  },[apiBase,botId]);
  const sharedAuth={state:authState,set:(s)=>setAuthState(s.state),token,setToken};
  return <>
    <ThemeLabColorBox apiBase={apiBase} botId={botId} frameRef={frameRef} onVars={onVars} sharedAuth={sharedAuth}/>
    <ThemeLabWordingBox apiBase={apiBase} botId={botId} frameRef={frameRef} sharedAuth={sharedAuth}
      onFormfillChange={onFormfillChange} onLiveMessages={onLiveMessages}/>
  </>;
}
...
function OptionButton({opt,selected,onClick}){
  return (
    <button onClick={()=>onClick(opt)}
      className={classNames("w-full text-center rounded-[0.75rem] px-4 py-3 transition",
      ...
      {opt.tooltip && <div className="mt-1 text-[0.7rem] sm:text-[0.75rem] opacity-90">{opt.tooltip}</div>}
    </button>
  );
}
function QuestionBlock({q,value,onPick}){
  ...
  return (
    <div className="w-full rounded-[0.75rem] px-4 py-3 text-base bg-[var(--card-bg)] border border-[var(--border-default)]">
      ...
      {q.help_text && <div className="text-xs italic mt-1 text-[var(--helper-fg)]">{q.help_text}</div>}
      {opts.length>0 ? (
...
function PriceMirror({lines}){ if(!lines?.length) return null; return <div className="mb-3">{lines.map((ln,i)=><div key={i} className="text-base italic whitespace-pre-line text-[var(--mirror-fg)]">{ln}</div>)}</div>; }
function EstimateCard({estimate,outroText}){
  ...
  const fmtRange=(c,min,max)=>Number(min)===Number(max)?fmtAmount(c,max):`${fmtAmount(c,min)} – ${fmtAmount(c,max)}`;
    ...
    <div>
      <div className="rounded-[0.75rem] p-4 bg-white [box-shadow:var(--shadow-elevation)]">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="font-bold text-lg text-right [font-variant-numeric:tabular-nums]">{totalText}</div>
        </div>
            ...
            return (
              <div key={key} className="rounded-[0.75rem] p-3 bg-white">
                <div className="grid grid-cols-[1fr_auto] items-center gap-3">
                  ...
                  <div className="font-bold text-lg text-right [font-variant-numeric:tabular-nums]">{lineText}</div>
                </div>
              </div>
            );
          })}
      ...
      {outroText && <div className="mt-3 text-base font-bold whitespace-pre-line">{outroText}</div>}
    </div>
  );
}
...
export default function Welcome() {
  ...
  const { alias, botIdFromUrl }=useMemo(()=>{
    ...
    return { alias:(qs.get("alias")||qs.get("alais")||"").trim(), botIdFromUrl:(qs.get("bot_id")||"").trim() };
  },[]);
  ...
  const derivedTheme=useMemo(()=>({...themeVars,"--tab-active-fg":inverseBW(themeVars["--tab-fg"]||"#000")}),[themeVars]);
  ...
  const FORM_KEY=useMemo(()=>`formfill_completed:${botId||alias||"_"}`,[botId,alias]);
  useEffect(()=>{ try{ if(sessionStorage.getItem(FORM_KEY)==="1") setFormCompleted(true);}catch{} },[FORM_KEY]);
  ...
  function maybePrefillFirstQuestion(q){
    if(!q||firstPrefillDone.current) return;
    setInput(curr=>{
      if(curr.trim().length===0){ firstPrefillDone.current=true; return q;}
      return curr;
    });
  }
  function enrichPerspectiveIfPresent(fields, defaults){
    return {
      fields:(fields||[]).map(f=>f.field_key==="perspective" ? {
        ...f,
  ...
  async function refetchVisitorValues(){
    if(!visitorId) return;
    try{
      const r=await fetch(`${apiBase}/visitor-formfill?visitor_id=${encodeURIComponent(visitorId)}`);
      if(!r.ok) return;
      const j=await r.json();
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        const r=await fetch(`${apiBase}/bot-settings?alias=${encodeURIComponent(alias)}`);
        const j=await r.json();
        if(cancel) return;
        ...
        if(id) setBotId(id); else if(!r.ok || j?.ok===false) setFatal("Invalid or inactive alias.");
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        const r=await fetch(`${apiBase}/bot-settings?alias=${encodeURIComponent(defaultAlias)}`);
        const j=await r.json();
        if(cancel) return;
        if(j?.ok){ setVisitorId(j.visitor_id||""); setSessionId(j.session_id||""); }
        const b=j?.ok?j?.bot:null;
        ...
        if(j?.ok && j?.bot?.id) setBotId(j.bot.id);
      }catch{}
    })();
    return ()=>{cancel=true;};
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        const r=await fetch(`${apiBase}/brand?bot_id=${encodeURIComponent(botId)}`);
        const j=await r.json();
        if(cancel) return;
        if(j?.ok && j?.css_vars) setThemeVars(p=>({...p,...j.css_vars}));
        if(j?.ok && j?.assets){
          setBrandAssets({
  ...
  async function fetchFormfillConfig(botIdArg, aliasArg){
    try{
      ...
      const r=await fetch(`${apiBase}/formfill-config?${params.toString()}`);
      const j=await r.json();
      if(j?.ok){
  ...
  function patchCanonicalFields(rawFields){
    ...
    rawFields.forEach(f=>{
      ...
      if(!existing) {
        ...
        existing.is_collected = existing.is_collected || base.is_collected;
        existing.is_required = existing.is_required || base.is_required;
        if(!existing.tooltip && base.tooltip) existing.tooltip=base.tooltip;
        if(!existing.placeholder && base.placeholder) existing.placeholder=base.placeholder;
        if(!existing.field_type && base.field_type) existing.field_type=base.field_type;
        if(!existing.options && base.options) existing.options=base.options;
      }
    });
    return [...map.values()];
  }

  useEffect(()=>{ if(botId) fetchFormfillConfig(botId,null); },[botId]);
  useEffect(()=>{ if(!botId && alias) fetchFormfillConfig(null,alias); },[alias,botId]);
  useEffect(()=>{ if(botId && visitorId) fetchFormfillConfig(botId,null); },[visitorId,botId]);
  useEffect(()=>{ if((mode==="formfill"||mode==="personalize") && visitorId && botId) refetchVisitorValues(); },[mode,visitorId,botId]);

  const activeFormFields=useMemo(()=> (formFields||[]).map(f=>f.field_key==="perspective"?{
    ...f, field_type:"single_select", options:f.options && f.options.length? f.options : PERSPECTIVE_OPTIONS
  }:f),[formFields]);
  ...
  async function doSend(outgoing){
    ...
    try{
      ...
      const status=res.status; const data=res.data||{};
      const ok=data.ok!==false && status>=200 && status<300 && typeof data.response_text==="string";
      if(!ok){
        const msg=data.response_text||data.message||(status===500?"Internal server error":`Request failed (${status})`);
        setResponseText(msg||"Sorry—something went wrong."); setLoading(false);
        setLastError({status,data,payloadSent:payload}); return;
      }
      ...
      const fallback=data?.response_text||data?.message||(status?`Request failed (HTTP ${status})`:"Network error");
      setResponseText(fallback.startsWith("Request failed")?"Sorry—something went wrong.":fallback);
      setLastError({status,data,message:err.message,stack:err.stack,payloadSent:payload});
  ...
  function maybeOpenForm(next){
    ...
    if(!formCompleted && !formShown){
      setFormShown(true); setPending(next); setMode("formfill"); return true;
    }
    return false;
  }
  async function onSendClick(){
    const outgoing=input.trim(); if(!outgoing||!botId) return;
    if(maybeOpenForm({type:"ask",payload:{text:outgoing}})) return;
    await doSend(outgoing);
  }
  function openPersonalize(){ refetchVisitorValues(); setPending(null); setMode("formfill"); setFormShown(true); }
    ...
    try{
      const r=await fetch(`${apiBase}/render-video-iframe`,{
        ...
        body:JSON.stringify(withIdsBody({
          bot_id:botId,demo_id:item.id||"",title:item.title||"",video_url:item.url||""
        }))
      });
      const j=await r.json();
  ...
  async function _openMeeting(){
    ...
    try{
      const r=await fetch(`${apiBase}/agent?bot_id=${encodeURIComponent(botId)}`);
      const j=await r.json(); const ag=j?.ok?j.agent:null; setAgent(ag);
  ...
  function _openPrice(){ if(!botId) return; setMode("price"); setSelected(null); }
  function openPrice(){ if(maybeOpenForm({type:"price"})) return; _openPrice(); }
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        const r=await fetch(`${apiBase}/pricing/questions?bot_id=${encodeURIComponent(botId)}`);
        const j=await r.json();
        if(cancel) return;
        if(!j?.ok) throw new Error();
        setPriceQuestions(Array.isArray(j.questions)?j.questions:[]);
        requestAnimationFrame(()=>contentRef.current?.scrollTo({top:0,behavior:"auto"}));
        setPricingCopy(prev=>({
          intro:j.pricing_intro||prev.intro,
          outro:j.pricing_outro||prev.outro,
          custom_notice:j.pricing_custom_notice||prev.custom_notice
        }));
      }catch{ if(!cancel) setPriceErr("Unable to load price estimator."); }
    })();
    return ()=>{cancel=true;};
  },[mode,botId,apiBase]);
  ...
  useEffect(()=>{
    if(mode!=="price"||!botId||nextPriceQuestion){
      setPriceEstimate(prev=>nextPriceQuestion?null:prev); return;
    }
    let cancel=false;
    (async()=>{
      try{
        ...
        const body={
          bot_id:botId,
          answers:{
            product_id: priceAnswers?.product || priceAnswers?.edition || priceAnswers?.product_id || "",
            tier_id: priceAnswers?.tier || priceAnswers?.transactions || priceAnswers?.tier_id || "",
          },
        ...
        if(!j?.ok) throw new Error();
        setPriceEstimate(j);
      }catch{ if(!cancel) setPriceErr("Unable to compute estimate."); }
      finally{ if(!cancel) setPriceBusy(false); }
    })();
    return ()=>{cancel=true;};
  },[mode,botId,apiBase,priceQuestions,priceAnswers,nextPriceQuestion,sessionId,visitorId]);
  ...
  const mirrorLines=useMemo(()=>{
    ...
    if(typeof priceEstimate?.mirror_text==="string"){
      const t=priceEstimate.mirror_text.trim(); if(t) return [t];
    }
    if(Array.isArray(priceEstimate?.mirror_text)){
      const out=[];
      for(const m of priceEstimate.mirror_text){
        const raw=String(m?.text||"").trim(); if(!raw) continue;
        const lbl=labelFor(m?.q_key);
        const rep=raw.replace(/\{\{\s*answer_label_lower\s*\}\}/gi,lbl.toLowerCase())
    ...
    for(const q of priceQuestions){
      ...
      if(tmpl && typeof tmpl==="string"){
        const rep=tmpl.replace(/\{\{\s*answer_label_lower\s*\}\}/gi,label.toLowerCase())
  ...
  function handleLiveMessages(updated){
    ...
    setPricingCopy(prev=>({
      ...
      custom_notice:"pricing_custom_notice" in updated ? updated.pricing_custom_notice||"" : prev.custom_notice,
    }));
  }

  if(fatal){
    return <div className="w-screen min-h-[100dvh] flex items-center justify-center bg-gray-100 p-4">
    ...
    return (
      <div className={classNames("w-screen min-h-[100dvh] flex items-center justify-center bg-[var(--page-bg)] p-4 transition-opacity duration-200",brandReady?"opacity-100":"opacity-0")} style={liveTheme}>
        <div className="text-gray-800 text-center space-y-2">
          ...
          {alias ? <div className="text-sm text-gray-600">Resolving alias “{alias}”…</div> :
  ...
  function handleThemeLabFormfillChange({show_formfill,standard_fields}) {
    ...
    if(Array.isArray(standard_fields)){
      setFormFields(prev=>{
        const byKey={};
        prev.forEach(f=>{ if(f?.field_key) byKey[f.field_key]={...f}; });
          ...
          <div className="flex items-center justify-between w-full py-3">
            ...
            <div className="text-lg sm:text-xl font-semibold truncate max-w-[60%] text-right">
              {selected?selected.title:
                mode==="personalize"||mode==="formfill"?"Personalize":
                mode==="browse"?"Browse Demos":
                mode==="docs"?"Browse Documents":
                mode==="meeting"?"Schedule Meeting":
                mode==="price"?"Price Estimate":"Ask the Assistant"}
            </div>
          </div>
          ...
          {(mode==="formfill"||mode==="personalize") ? (
            <div className="space-y-4">
              ...
              <FormFillCard
                ...
                onSubmit={async vals=>{
                  if(!visitorId) return;
                  if(typeof vals.perspective==="string") vals.perspective=vals.perspective.toLowerCase();
                  updateLocalVisitorValues(vals);
                  let postOk=false;
                  try{
                    const resp=await fetch(`${apiBase}/visitor-formfill`,{
                      method:"POST",headers:{"Content-Type":"application/json"},
                      body:JSON.stringify({visitor_id:visitorId,values:vals,bot_id:botId||undefined})
                    });
                    postOk=resp.ok;
                  }catch{}
                  try{ sessionStorage.setItem(FORM_KEY,"1"); }catch{}
              ...
              <div className="pt-0 pb-0">
                ...
                {!mirrorLines.length && (
                  <div className="text-base font-bold whitespace-pre-line">
                    {pricingCopy.intro || "This tool provides a quick estimate based on your selections. Final pricing may vary."}
                  </div>
                )}
              </div>
              <div className="mt-2 space-y-4">
                {!priceQuestions.length ? <div className="text-sm text-[var(--helper-fg)]">Loading questions…</div>
                  : nextPriceQuestion ? <QuestionBlock q={nextPriceQuestion} value={priceAnswers[nextPriceQuestion.q_key]} onPick={handlePickPriceOption}/>
                  : priceEstimate && priceEstimate.custom ? (
                    <div className="text-base font-bold whitespace-pre-line">
                      {pricingCopy.custom_notice || "We’ll follow up with a custom quote tailored to your selection."}
                    </div>
                  ) : <EstimateCard estimate={priceEstimate} outroText={pricingCopy.outro||""}/>
                }
                {!nextPriceQuestion && priceBusy && <div className="text-sm text-[var(--helper-fg)]">Calculating…</div>}
                {priceErr && <div className="text-sm text-red-600">{priceErr}</div>}
              </div>
            </div>
          ) : mode==="meeting" ? (
            <div className="w-full flex-1 flex flex-col">
              {agent?.schedule_header && <div className="text-sm italic text-[var(--helper-fg)] mb-3 whitespace-pre-line">{agent.schedule_header}</div>}
              {!agent ? <div className="text-sm text-[var(--helper-fg)]">Loading scheduling…</div>
            ...
            <div className="w-full flex-1 flex flex-col">
              {mode==="docs" ? <DocIframe doc={selected}/> : (
                <div className="bg-[var(--card-bg)] pt-2 pb-2">
                  <iframe style={{width:"100%",aspectRatio:"471 / 272"}} src={selected.url} title={selected.title}
                    className="rounded-[0.75rem] [box-shadow:var(--shadow-elevation)]"
              ...
              {mode==="ask" && (visibleUnderVideo||[]).length>0 && (
                <>
                  ...
                  <div className="flex flex-col gap-3">
                    {visibleUnderVideo.map(it=><Row key={it.id||it.url||it.title} item={it} onPick={val=>normalizeAndSelectDemo(val)}/>)}
                  </div>
                </>
              )}
            </div>
            ...
            <div className="w-full flex-1 flex flex-col">
              {(browseItems||[]).length>0 && (
                <>
                  ...
                  <div className="flex flex-col gap-3">
                    {browseItems.map(it=> <Row key={it.id||it.url||it.title} item={it} onPick={val=>normalizeAndSelectDemo(val)}/>)}
                  </div>
                </>
              )}
            </div>
            ...
            <div className="w-full flex-1 flex flex-col">
              {(browseDocs||[]).length>0 && (
                <>
                  ...
                  <div className="flex flex-col gap-3">
                    {browseDocs.map(it=> <Row key={it.id||it.url||it.title} item={it} kind="doc"
                      onPick={async val=>{
                        try{
                          const r=await fetch(`${apiBase}/render-doc-iframe`,{
                            method:"POST",headers:{"Content-Type":"application/json"},
                            body:JSON.stringify(withIdsBody({
                              bot_id:botId,doc_id:val.id||"",title:val.title||"",url:val.url||""
                            }))
                          });
                          const j=await r.json();
                          setSelected({...val,_iframe_html:j?.iframe_html||null});
                        }catch{ setSelected(val);}
                        requestAnimationFrame(()=>contentRef.current?.scrollTo({top:0,behavior:"auto"}));
                      }}/>)}
                  </div>
                </>
            ...
            <div className="w-full flex-1 flex flex-col">
              ...
              {lastQuestion && <p className="text-base italic text-center mb-2 text-[var(--helper-fg)]">"{lastQuestion}"</p>}
              <div className="text-left mt-2">
                {loading ? <p className="font-semibold animate-pulse text-[var(--helper-fg)]">Thinking…</p>
                  : lastQuestion ? <p className="text-base font-bold whitespace-pre-line">{responseText}</p> : null}
              </div>
              {(items||[]).length>0 && (
                <>
                  ...
                  <div className="flex flex-col gap-3">
                    {items.map(it=> <Row key={it.id||it.url||it.title} item={it} onPick={val=>normalizeAndSelectDemo(val)}/>)}
                  </div>
                </>
              )}
              {lastError && (
                <details className="mt-4 text-[11px] p-2 border border-red-300 rounded bg-red-50">
                  <summary className="cursor-pointer text-red-700 font-semibold">Technical details</summary>
...
{JSON.stringify(lastError,null,2)}
        ...
        {showAskBottom && (
          <AskInputBar value={input} onChange={setInput} onSend={onSendClick}
            inputRef={inputRef} placeholder="Ask your question here"/>
        )}
      </div>
